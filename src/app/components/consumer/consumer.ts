...
import { addDiscount, addToCart, createCart, getActiveCart } from '../../utils/api/api-cart';
...
export class Consumer implements Observable {
  ...
  async init(): Promise<void> {
    ...
    if (this.isConsumer) {
      ...
      this.cart = (await addDiscount(this.apiClient, this.cart.version, this.cart.id, 'WAVE2023')).body;
      this.cart = (await addDiscount(this.apiClient, this.cart.version, this.cart.id, 'FIRST1PULSE')).body;
      this.cart = (await addDiscount(this.apiClient, this.cart.version, this.cart.id, 'ATHLETE10')).body;

      this.cart = (await addToCart(this.apiClient, this.cart.version, this.cart.id, '4ce336b4-1207-4288-b9ac-93f774deed71')).body;
      this.cart = (await addDiscount(this.apiClient, this.cart.version, this.cart.id, 'CIRCUS')).body;
    }
